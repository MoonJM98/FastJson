...
{
    public static string EmitContext(EquatableArray<TypeModel> types, FastJsonOptionsModel options, string assemblyName)
    {
        var sb = new StringBuilder();

        // Create a safe namespace suffix from assembly name
        var safeAssemblyName = GetSafeIdentifier(assemblyName);
        var generatedNamespace = $"FastJson.Generated.{safeAssemblyName}";
        ...
        // Emit inlinable FastJson wrapper class
        EmitFastJsonWrapper(sb, types);

        return sb.ToString();
    }

    private static void EmitFastJsonWrapper(StringBuilder sb, EquatableArray<TypeModel> types)
    {
        sb.AppendLine("/// <summary>");
        sb.AppendLine("/// High-performance FastJson wrapper with inlinable serialize/deserialize methods.");
        sb.AppendLine("/// Use this class instead of global::FastJson.FastJson for maximum performance.");
        sb.AppendLine("/// </summary>");
        sb.AppendLine("public static class FastJson");
        sb.AppendLine("{");
        sb.AppendLine("    [ThreadStatic] private static global::FastJson.PooledBufferWriter? _buffer;");
        sb.AppendLine();

        // Emit Serialize<T>
        EmitSerializeWrapper(sb, types);

        // Emit Deserialize<T>
        EmitDeserializeWrapper(sb, types);

        // Emit SerializeToUtf8Bytes<T>
        EmitSerializeToUtf8BytesWrapper(sb, types);

        sb.AppendLine("}");
    }

    private static void EmitSerializeWrapper(StringBuilder sb, EquatableArray<TypeModel> types)
    {
        sb.AppendLine("    /// <summary>");
        sb.AppendLine("    /// Serializes the specified value to a JSON string. Inlinable for registered types.");
        sb.AppendLine("    /// </summary>");
        ...
        foreach (var type in types)
        {
            ...
            sb.AppendLine("        {");
            sb.AppendLine("            var buffer = _buffer ??= new global::FastJson.PooledBufferWriter(256);");
            sb.AppendLine("            buffer.Clear();");
            sb.AppendLine("            using var writer = new Utf8JsonWriter(buffer);");
            sb.AppendLine($"            {safeTypeName}Serializer.Write(writer, ({type.FullyQualifiedName})(object)value!);");
            sb.AppendLine("            writer.Flush();");
            sb.AppendLine("            return global::System.Text.Encoding.UTF8.GetString(buffer.WrittenSpan);");
            sb.AppendLine("        }");
        }

        sb.AppendLine("        return global::FastJson.FastJson.Serialize(value);");
        sb.AppendLine("    }");
        sb.AppendLine();
    }

    private static void EmitDeserializeWrapper(StringBuilder sb, EquatableArray<TypeModel> types)
    {
        sb.AppendLine("    /// <summary>");
        sb.AppendLine("    /// Deserializes the specified JSON string to an object. Inlinable for registered types.");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    [MethodImpl(MethodImplOptions.AggressiveInlining)]");
        ...
        foreach (var type in types)
        {
            // Skip anonymous types (write-only)
            if (type.IsAnonymous)
                continue;
            ...
            sb.AppendLine($"        if (typeof(T) == typeof({type.FullyQualifiedName}))");
            sb.AppendLine("        {");
            sb.AppendLine("            var bytes = global::System.Text.Encoding.UTF8.GetBytes(json);");
            sb.AppendLine("            var reader = new Utf8JsonReader(bytes);");
            sb.AppendLine($"            return (T?)(object?){safeTypeName}Deserializer.Read(ref reader);");
            sb.AppendLine("        }");
        }

        sb.AppendLine("        return global::FastJson.FastJson.Deserialize<T>(json);");
        sb.AppendLine("    }");
        sb.AppendLine();
    }

    private static void EmitSerializeToUtf8BytesWrapper(StringBuilder sb, EquatableArray<TypeModel> types)
    {
        sb.AppendLine("    /// <summary>");
        sb.AppendLine("    /// Serializes the specified value to a UTF-8 byte array. Inlinable for registered types.");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    [MethodImpl(MethodImplOptions.AggressiveInlining)]");
        sb.AppendLine("    public static byte[] SerializeToUtf8Bytes<T>(T value)");
        sb.AppendLine("    {");
        ...
        sb.AppendLine("        return global::FastJson.FastJson.SerializeToUtf8Bytes(value);");
        sb.AppendLine("    }");
        sb.AppendLine();
    }
    ...
    {
        // Emit the FastJsonContext class that implements IFastJsonContext
        EmitFastJsonContext(sb, types);
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        global::FastJson.FastJson.RegisterContext(FastJsonContext.Instance);");
        sb.AppendLine("    }");
        sb.AppendLine("}");
    }

    private static void EmitFastJsonContext(StringBuilder sb, EquatableArray<TypeModel> types)
    {
        sb.AppendLine("/// <summary>");
        sb.AppendLine("/// FastJson context that provides serializers and deserializers for types in this assembly.");
        sb.AppendLine("/// </summary>");
        sb.AppendLine("internal sealed class FastJsonContext : global::FastJson.IFastJsonContext");
        sb.AppendLine("{");
        sb.AppendLine("    public static readonly FastJsonContext Instance = new();");
        sb.AppendLine();
        sb.AppendLine("    [ThreadStatic] private static global::FastJson.PooledBufferWriter? _buffer;");
        sb.AppendLine();
        sb.AppendLine("    private FastJsonContext() { }");
        sb.AppendLine();

        // TryGetWriter<T>
        sb.AppendLine("    public bool TryGetWriter<T>(out Action<Utf8JsonWriter, T>? writer)");
        sb.AppendLine("    {");
        ...
        {
            var safeTypeName = GetSafeTypeName(type);
            var prefix = firstWriter ? "if" : "else if";
            firstWriter = false;

            sb.AppendLine($"        {prefix} (typeof(T) == typeof({type.FullyQualifiedName}))");
            sb.AppendLine("        {");
            sb.AppendLine($"            writer = (Action<Utf8JsonWriter, T>)(object)(Action<Utf8JsonWriter, {type.FullyQualifiedName}>){safeTypeName}Serializer.Write;");
            sb.AppendLine("            return true;");
            sb.AppendLine("        }");
        }

        sb.AppendLine();
        sb.AppendLine("        writer = null;");
        sb.AppendLine("        return false;");
        sb.AppendLine("    }");
        sb.AppendLine();

        // TryGetReader<T>
        sb.AppendLine("    public bool TryGetReader<T>(out global::FastJson.FastJsonReadDelegate<T>? reader)");
        sb.AppendLine("    {");
        ...
        foreach (var type in types)
        {
            ...
            var safeTypeName = GetSafeTypeName(type);
            var prefix = firstReader ? "if" : "else if";
            firstReader = false;

            sb.AppendLine($"        {prefix} (typeof(T) == typeof({type.FullyQualifiedName}))");
            sb.AppendLine("        {");
            sb.AppendLine($"            reader = (global::FastJson.FastJsonReadDelegate<T>)(object)(global::FastJson.FastJsonReadDelegate<{type.FullyQualifiedName}>){safeTypeName}Deserializer.Read;");
            sb.AppendLine("            return true;");
            sb.AppendLine("        }");
        }

        sb.AppendLine();
        sb.AppendLine("        reader = null;");
        sb.AppendLine("        return false;");
        sb.AppendLine("    }");
        sb.AppendLine();

        // Serialize<T> - inline method
        sb.AppendLine("    [MethodImpl(MethodImplOptions.AggressiveInlining)]");
        sb.AppendLine("    public string? Serialize<T>(T value)");
        sb.AppendLine("    {");
        ...
        {
            var safeTypeName = GetSafeTypeName(type);
            var prefix = firstSerialize ? "if" : "else if";
        ...
        // Deserialize<T> - inline method
        sb.AppendLine("    [MethodImpl(MethodImplOptions.AggressiveInlining)]");
        sb.AppendLine("    public (bool Success, T? Value) Deserialize<T>(string json)");
        sb.AppendLine("    {");
        ...
        foreach (var type in types)
        {
            ...
            var safeTypeName = GetSafeTypeName(type);
            var prefix = firstDeserialize ? "if" : "else if";
            ...
            sb.AppendLine("            var bytes = global::System.Text.Encoding.UTF8.GetBytes(json);");
            sb.AppendLine("            var reader = new Utf8JsonReader(bytes);");
            sb.AppendLine($"            return (true, (T?)(object?){safeTypeName}Deserializer.Read(ref reader));");
            sb.AppendLine("        }");
        }
    ...
    /// <summary>
    /// Creates a safe C# identifier from a string (e.g., assembly name).
    /// </summary>
